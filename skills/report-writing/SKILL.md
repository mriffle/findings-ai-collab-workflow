...
- **Figures** = the exact artifacts the findings already point to (svg/png + legend), **embedded inline — legend image directly beneath the figure — and explained** — show, don't tell: a claim the source finding illustrated stays illustrated, and each figure keeps its legend, its caption, plus a one-or-two-sentence reading in the prose (`conventions/findings.md` §9).
