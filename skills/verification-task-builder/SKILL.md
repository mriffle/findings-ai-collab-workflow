...
- `figures` — **the whole block, plus the body's inline images and their readings.** A figure *is* the answer rendered: a volcano shows the direction and the hit count, an ROC shows the performance, and the five-part body pattern puts a plain-language **reading** right beside each image and its legend (`conventions/findings.md` §9). Never pass a figure path, caption, or reading to the verifier, and never point it at this finding's `<NNNN>-*` artifacts anywhere under `figures/` — it renders its own figures if it wants them.
