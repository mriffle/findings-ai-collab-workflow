...
- **The shuffle null is a second pass, not part of the first.** Run the model **without** it initially (`run_null=False` — the template default) so the scientist sees performance, coefficients, and figures quickly; the null roughly **3.5×'s** the run (the shrinkage LDA's is cheap — run its null right after the first pass, in the same sitting). Then **propose it as the immediate follow-up**: permute the target, re-run at the fixed hyperparameters, confirm performance collapses to chance, and report the null distribution + empirical p beside the real score. Until it runs, the finding is **`exploratory`** and the coefficients are flagged "not tested against a null." On a grouped design the null's scheme follows the unit — `null_permutation="units"` (default) when each unit carries one class, `"within_units"` for batches holding both classes (labels shuffled inside each batch). Do not skip it quietly — it is the **leakage detector** too (if performance doesn't collapse, there is leakage or no real signal), so it is required before any strong claim.
- **A second linear classifier is a triggered, paired comparison — not a tournament.** The elastic net is the reference; the linear SVM or the shrinkage LDA runs beside it only on a named trigger, with the reading wanted stated first, the alternative fitted cleanly (SVM tuning warnings resolved; LDA shrinkage saturation acknowledged), and the decision made on the **paired per-fold comparison** at the same seed — sign consistency across seeds, not the higher mean; every AUC reported (`conventions/statistics.md`, *Choosing among the linear classifiers*).
