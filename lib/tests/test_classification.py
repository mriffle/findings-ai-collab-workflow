...
def test_roc_legend_auc_equals_cv_auc(planted_result: cls.ClassificationResult) -> None:
...
def test_null_line_at_observed_auc() -> None:
    ...
    res = cls.classify(ds, "grp", run_null=True, n_permutations=3, **_NULL_FAST)
...
def test_heatmap_box_marks_best_cell(planted_result: cls.ClassificationResult) -> None:
    from matplotlib.patches import Rectangle

    fig = clsfig.plot_hyperparameter_heatmap(planted_result)
    rects = [p for p in fig.axes[0].patches if isinstance(p, Rectangle)]
    assert len(rects) == 1
    row = list(planted_result.c_grid).index(planted_result.best_c)
    col = list(planted_result.l1_grid).index(planted_result.best_l1_ratio)
...
def test_heatmap_colorbar_names_tuning_metric(
    ...
    plt.close(fig)
    fig = clsfig.plot_hyperparameter_heatmap(
...
def test_feature_list_caveat_survives_custom_title(
    ...
    for fig in (
        clsfig.plot_roc(res, title="MY TITLE"),
        clsfig.plot_null(res, title="MY TITLE"),
        clsfig.plot_coefficients(res, title="MY TITLE"),
        clsfig.plot_hyperparameter_heatmap(res, title="MY TITLE"),
    ):
...
def test_figure_error_path_closes(
    planted_result: cls.ClassificationResult, monkeypatch: pytest.MonkeyPatch
) -> None:
    plt.close("all")
    ...
    monkeypatch.setattr(clsfig, "_apply_title", boom)
    for plot in (
        clsfig.plot_roc,
        clsfig.plot_null,
        clsfig.plot_coefficients,
        clsfig.plot_hyperparameter_heatmap,
    ):
...
def test_within_unit_null_guards() -> None:
    ...
    with pytest.warns(cls.NullPermutationWarning, match="16 distinct"):
        res = cls.classify(
            small,
            "grp",
            groups="batch",
