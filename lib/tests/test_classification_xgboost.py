...
def test_grouping_engaged_with_repeats() -> None:
    ...
    # _planted alternates y, so each consecutive pair is a MIXED unit (the null is
    # not run here — see the within-unit tests below)
...
def test_within_unit_permutation_preserves_unit_counts() -> None:
    ...
    changed = False
    for _ in range(50):
        perm = xgb._permute_labels(y, g, rng, "within_units")
        for u in ("a", "b", "c"):
    ...
    with pytest.raises(ValueError, match="within_units"):
        xgb._permute_labels(y, g, rng, "units")
    # ungrouped: a plain row shuffle whatever the scheme
    plain = xgb._permute_labels(y, None, np.random.default_rng(0), "within_units")
...
def test_within_unit_arrangements_hand_computed() -> None:
    ...
    g = np.array(["a"] * 6 + ["b"] * 6)
    assert xgb._within_unit_arrangements(y, g) == 400
    # every unit pure -> exactly one arrangement (the identity)
    assert (
        xgb._within_unit_arrangements(np.array([0, 0, 1, 1]), np.array(list("aabb")))
        == 1
    )
    ...
    assert xgb._within_unit_arrangements(y6, g6) == 184756**6
...
def test_within_unit_null_guards() -> None:
    ...
    with pytest.raises(ValueError, match="needs grouped CV"):
        xgb.classify_xgboost(ds, "grp", **kw)
    ...
    with (
        pytest.warns(xgb.SingletonGroupsWarning),
        pytest.raises(ValueError, match="'unit'"),
    ):
        xgb.classify_xgboost(single, "grp", groups="unit", **kw)
...
def test_null_permutation_recorded_and_titled() -> None:
    ds = _planted(n=40, p=20, n_signal=4, seed=0)
    assert (
        xgb.classify_xgboost(ds, "grp", **_NULL_FAST).null_permutation is None
    )  # null not run
    rows = xgb.classify_xgboost(
        ds, "grp", run_null=True, n_permutations=3, **_NULL_FAST
    )
    assert rows.null_permutation == "samples"  # row-level CV: a plain shuffle
    units = xgb.classify_xgboost(
        _pure_units(_planted(n=40, p=20, n_signal=4, seed=0)),
        "grp",
        groups="animal",
        run_null=True,
    ...
    for res, needle in (
        ...
        plt.close(fig)
    assert "unit-level" not in xgbfig.plot_null(rows).get_suptitle()
    plt.close("all")
...
def test_grouped_null_rejects_mixed_label_unit() -> None:
    ...
    with pytest.raises(ValueError, match="both classes"):
        xgb._permute_labels(y, g, np.random.default_rng(0))
