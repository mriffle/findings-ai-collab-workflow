...
Four figures read a :class:`~analysis.classification.ClassificationResult`:
  ...
  * :func:`plot_roc` — the mean ROC across outer nested-CV folds with a ±1 SD band and a
    ...
    plot region. The legend sits on-axes (lower-right, where a good classifier leaves
    space) — a documented exception to the separate-legend convention, like the
    correlation r-colorbar (conventions/visualization.md).
  ...
  * :func:`plot_coefficients` — the top-N selected features by |all-data standardized
    coefficient|, each a diamond at its final coefficient over the IQR of its
    coefficient across the resamples that **selected** it (the analysis masks the
    non-selecting resamples before taking quartiles, so a feature selected once draws
    a zero-width bar), colored by **selection frequency** (viridis). A vertical line
    at 0 separates the classes; the colorbar is the stability read. Selected-only (no
    rejected features), so it shares a *skeleton* with the Boruta importance plot,
    not an identical look.
  * :func:`plot_hyperparameter_heatmap` — the all-data tuning surface (the mean
    inner-CV ``tuning_metric``, AUC by default, over the C x l1_ratio grid) with the
    selected cell boxed. A diagnostic.
...
__script_meta__: dict[str, object] = {
    ...
    "description": (
        ...
        "(legend on-axes carrying the result's nested-CV AUC; the summary numbers in "
        "a caption strip below the axes, never over the plot), the label-shuffle "
        "null AUC histogram (conditional on the null being run), the top-N "
        "coefficient plot (all-data coef + IQR over the resamples that selected it, "
        "colored by selection frequency on viridis, selected-only), and the C x "
        "l1_ratio hyperparameter heatmap with the selected cell boxed and the "
        "tuning metric named. Dual-export via figure-io; colorbars beside the axes "
        "(no separate legend figure). Study-agnostic; fail-loud."
    ),
}
...
def _roc_summary(result: ClassificationResult) -> str:
    ...
    with publication_style():
        ...
        try:
            ...
            _apply_title(
                ...
                f"(diamond = all-data coef, bar = IQR over the resamples that "
                f"selected it)",
                result,
            )
