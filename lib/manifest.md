...
| normalize | 0.1 | lib/common/normalize.py | module | `NormalizationMethod`, `normalize`, `log2_transform` | Verified `Dataset`→`Dataset` normalization (pronoms median / MAD / VSN) + `log2(x+1)` transform. Scale-tag guarded against the double-log trap (refuses non-linear input), shape-preserving, fail-loud. Study-agnostic; sets the output `scale`. Requires `pronoms`. |
| batch-correct-combat | 0.1 | lib/common/batch_correct.py | module | `BatchConfoundingWarning`, `ConfoundingReport`, `assess_batch_confounding`, `combat_correct` | Verified ComBat batch correction on a `Dataset` — **batch label only** (no covariate preserved, a deliberate anti-cheating choice), log-scale guarded, zero-variance-in-batch passthrough, `sample_mask`, fail-loud. Ships `assess_batch_confounding` which warns loudly when the batch axis is confounded with a covariate of interest. Requires `pycombat`. |
