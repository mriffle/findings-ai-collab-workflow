...
**Status 2026-09-17 (review follow-up, release 0.7.0).** A full review of the four classifier templates found and fixed: the inner tuning CV was never group-aware (a leaked tuning surface under replicates — now `StratifiedGroupKFold` inside, `inner_cv_grouped` recorded); the XGBoost tuning table was reshaped against `ParameterGrid`'s key order (scrambled `best_params` — now indexed by the recorded values); `select="smoothed"` applied only to the all-data fit (now one rule, `refit=False`, in every loop); a pandas-3 NA crash in label/group resolution; a NaN null score reading as `p = 1/(n+1)`; `generalization_target` accepted without `groups=`; the LDA's balanced accuracy at the prior-shifted 0 (now the equal-prior cut) and its silent two-row class under grouping; XGBoost's per-split `gain` default (now `total_gain`) and float `max_depth`. **The paired comparison this section's evidence relied on now ships as `lib/analysis/classifier-comparison`** (`compare_classifiers` / `compare_across_seeds`: fold alignment by identity, the corrected resampled t as the decisive p with per-fold `n_test/n_train`, a Wilcoxon on the non-zero differences labelled indicative — so the Wilcoxon p = 0.002 quoted above is indicative, not decisive — and the sign-across-seeds verdict); the tree model joins the rule with reading `"non-linear"`. Deferred: an XGBoost grid redesign — at proteomics *n* the depth axis is degenerate (with `min_child_weight=1` under the logistic loss deeper cells score identically) and the effective stopping rule is the `min_child_weight` × hessian interaction, now documented in the template.
