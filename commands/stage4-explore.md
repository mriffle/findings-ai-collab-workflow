...
1. **Spark discussion with vetted boilerplate.** Seed standard analyses from the `lib/` templates (differential abundance via moderated models, nonparametric tests, feature finding, leakage-safe classifiers, regression, dimensionality reduction) — copy the relevant template into `scripts/scratch/` and adapt it, or reuse the project's existing script for that analysis (one script per task) — to give the scientist something to react to. Starting from a vetted template beats generating fresh statistics code; models get assumptions and missingness handling wrong in ways that look fine.
