...
Produce descriptive QC so collaborators can trust the data, and save a QC report under `reports/`. The backbone is seven shipped `lib/figures/` templates — **seed each into `scripts/scratch/` and adapt**; routing through them carries the registry colors, the dual-export, and the separate legend image for free. The optional features×samples **missingness map** (a clustered presence heatmap) has no template — add it write-from-scratch (under `figures/qc/missingness-map/`) if a structural view is useful (the `missingness` template covers the per-feature completeness + MNAR views). These are **descriptive sanity checks, not findings** — findings are Stage 4.

**Dispatch one `figure-generator` per plot-family — never one subagent for the whole report.** Each item below is a **plot-family**: the **CV** family (the experimental overlay + one figure per control type), the **PCA** family (the state series in both colorings), and the five single-figure families (`id-depth`, `missingness`, `dynamic-range`, `abundance-boxplot`, `sample-correlation`). **Every family writes to `figures/qc/<family>/`** (`figures/qc/cv/`, `figures/qc/pca/`, `figures/qc/id-depth/`, …) — name that directory in the dispatch; processing state goes in the file stem, never a directory (`conventions/visualization.md`, *Where figures live*). For each family, dispatch a **fresh, isolated `figure-generator`** that reads **only the one `lib/` template it seeds from**, writes **one parameterized script** emitting that family's figures in a single run, renders them, and hands the PNG(s) to a **fresh `figure-reviewer`**; a FAIL goes back to that same family's generator. The orchestrator keeps only each family's returned **text contract** (base names, paths, encoding, verdict) and the accepted figure paths — **never the rendered images, templates, scripts, or run-output**. A single subagent producing all ~dozen QC figures accumulates every template + script + traceback + render and blows past a Pro-sized (~200k) context window; per-family fan-out keeps each context bounded and is why review is a separate agent (`figure-reviewer`), not an inline step.
...
**Prep the processing states *once*, before fanning out the plots.** Don't re-normalize and re-run ComBat inside every figure job. **First** dispatch a **`coder`** to build a small QC-prep script that loads the paired data (the Stage-3 loader), applies the confirmed normalization, previews batch correction if a ≥2-batch axis exists, and **materializes the canonical state matrices** with `dataset-io.save_dataset` under `results/qc_states/<state>/` — each on the scale its plots consume:

- `raw_linear` — the loaded matrix (id-depth, missingness, dynamic-range, CV-raw);
- `raw_log` — `log2_transform(raw_linear)` (PCA-raw);
- `normalized_linear` — `normalize(raw, median)` (CV-normalized);
- `normalized_log` — `log2_transform(normalized_linear)` (PCA-normalized, sample-correlation, abundance-boxplot);
- **if a ≥2-batch axis:** `batch_corrected_log` — `combat_correct(normalized_log, batch)` (PCA-BC, abundance-boxplot-BC);
- **if a ≥2-batch axis:** `batch_corrected_linear` — `to_linear(batch_corrected_log)` (CV-BC).

Materialize the **full cohort** (experimental + controls) in each state; a plot-family subsets to experimental / per-control-type in-memory after loading (cheap). **Then** fan out the plot-families (below): each `figure-generator` **`load_dataset`s the exact state(s) it needs** rather than re-deriving them — so ComBat runs **once**, not once per family, and each family's script shrinks to *load → subset → plot*. (The prep step is the one place normalization + the batch-correction preview are computed; the confounding assessment still runs once, below.)

**The required scale differs per plot — respect it or the templates refuse/mislead** (each maps to a prepared state above):
...
- **CV distributions (`cv-plot`) — on the LINEAR matrix.** Overlay per-feature CV (std/mean) across the processing states **raw → normalized → batch-corrected** — a drop in median CV is the evidence a step reduced nuisance variance. Emit the **whole family from one parameterized script** (one `figure-generator` dispatch), looping over the experimental subset and each control type. Two things to get right:
...
- **PCA (`pca-plot`) — on the LOG matrix.** PC1/PC2 (+ PC3/PC4) with marginal tests; **warns on a non-log scale**. Emit the **whole PCA family from one parameterized script** (one `figure-generator` dispatch) — **do not produce six ad-hoc figures.** Structure it as a **processing-state series rendered as small multiples, one figure per coloring** (so the family is **two figures**, not six):
  - **State series (small multiples) — `raw → normalized → batch-corrected`.** Within each figure, **one panel per state** (raw = log2 of the *unnormalized* matrix; normalized = log2 of the normalized matrix; batch-corrected = the ComBat output, already log), read left-to-right for the *effect* of each step — do the **control samples cluster more tightly** after normalization and again after batch correction, does a batch-driven split collapse. Use **PC1/PC2** for the state row; omit the batch-corrected panel when no correction was previewed (single batch). Produce the fuller PC1/PC2 **+ PC3/PC4** detail for the **normalized** state only (the one downstream analysis uses) if deeper inspection is wanted.
  - **Two colorings → two figures.** One small-multiple figure colored by the **batch axis** (batch structure — what the series is read against), one colored by **sample class with a distinct color per control type** — experimental, pooled-QC, reference, standard, … as *separate* registry categories (the >8-category guard applies), **not** a single control-vs-experimental split. Grey out reference samples only in the batch-axis figure where a plain background helps; for the class coloring we *want* the control colors visible.
