...
- **Confirm the data-checkable annotation markers are in the matrix.** Stage 1 listed the annotations a near-binary marker could check (`state/METADATA.md`, *data-checkable annotations* — Y-linked proteins for sex, a transgene or knockout product for genotype, …). Confirm each named marker feature is actually present, and at **which level** (protein group vs precursor / unique peptides — where a paralog shares most peptides, e.g. Y-linked DDX3Y vs X-linked DDX3X, only unique peptides can speak). Record presence/absence and the usable level in `state/DATA_DESCRIPTION.md`; an absent marker makes that annotation *not checkable* at Stage 3 — stated, not silently skipped. **Do not run the check here** — it needs the exact pairing Stage 3 builds.
...
These are *analysis* choices (not facts about the raw file), but they depend on what you find above, so surface them now and record the decision in `state/DATA_DESCRIPTION.md`. Each one is a **choice among known options**, so put it to the scientist with **`AskUserQuestion`** — the recommended option first, **one decision at a time** — rather than as a paragraph of alternatives to answer in one go (*How to ask*, project `CLAUDE.md`). They are applied later (QC in Stage 3, analysis in Stage 4), seeding from the `lib/` `normalize` and `batch-correct-combat` templates:
...
- **Missing-value handling.** If the matrix has missing values (NaN, distinct from a real `0` — settled in the encoding step above), the normalization and batch-correction templates require **complete data** and will fail loud on NaN. So the handling is a scientist decision to make here, not in the templates: present the options — **leave as `0`** (only if missing genuinely means not-detected/zero), **impute** (per-sample mean/median, or **KNN**), or **drop** features/samples above a missingness threshold — with their trade-offs, and record the choice. Apply the chosen handling with the vetted **`lib/common/missing-values`** template (`handle_missing`: an optional feature filter by maximum missing fraction, then a `zero` / `mean` / `median` / `knn` impute — the typical *filter-then-zero* recipe is one call) — it shares the `missingness`/`id-depth` detection predicate, runs on the **raw linear** matrix *before* normalization, and records the params (`max_missing_fraction`, `impute`, features dropped, values imputed) in the finding's `provenance.params`. The scientist owns the *choice*; the template owns the *application*. A left-censored imputer (MinProb/QRILC) for strong-MNAR data is not yet in the template — handle that case as project-local code for now. **Make this evidence-based, not blind:** the choice is *surfaced* here, but the evidence only exists once the matrix is loaded, so confirm/revisit it against the **Stage-3 `missingness` diagnostic** (`lib/figures/missingness`, in the QC report) — its **MNAR panel** (detection rate vs abundance) shows whether missingness is left-censored (low-abundance features dropping out ⇒ a left-censored imputation like MinProb/QRILC, *not* mean/median) or closer to random, and its **completeness curve** sets any drop threshold.
...
A verified description containing: orientation and shape; feature and sample identifier schemes; the **presence and usable level of each annotation marker** named in Stage 1 (or that none is present); transformation/normalization state (incl. the `scale` the loader will record); missing-value semantics; contaminant/decoy handling decisions; the **preprocessing decisions confirmed above** (normalization method; batch axis + any confound; missing-value handling); known data-quality issues; and the **data-version stamp**. Same regeneration and stamping discipline as `METADATA.md`.
