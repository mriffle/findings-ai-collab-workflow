...
**Two deliberate exceptions — the sample-correlation heatmap** (`lib/figures/sample-correlation`) **and the identification-depth bar chart** (`lib/figures/id-depth`): there, controls are shown *with* the experimental samples, distinguished by a **Sample Type label** (a correlation annotation stripe; a depth-bar color), because the cross-class comparison (do pools sit apart from biology / do they sit high and tight on depth? do references bridge batches?) *is* the deliverable — splitting them apart would destroy it. These are the only QC figures that mix the two classes, and only because they label them; the reviewer treats them as conforming.

## QC & exploration figures

The Stage 3 QC report (and the Stage 4 exploration seeds carried from it) is built on five shipped `lib/figures/` templates, each routing colors through the registry and dual-exporting with a separate legend. **Scale is load-bearing and differs per plot:**
...
| Control samples rendered separately from experimental in QC/descriptive figures (exceptions: the `sample-correlation` heatmap and the `id-depth` bar chart, which label them — a Sample Type stripe / bar color) | **Figure-reviewer** |
| QC plot scale correct (id-depth/CV linear; PCA/correlation log) | **Figure-reviewer** (+ `id-depth`/`cv-plot` refuse non-linear; `pca-plot`/`sample-correlation` warn on non-log) |
