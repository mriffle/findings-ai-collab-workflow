...
**Control samples are rendered separately from experimental samples** in QC and descriptive figures — their own panels or visibly distinct, never silently merged into the experimental distributions (a pool's tight cluster or a blank's empty profile would otherwise distort the very spread the plot is meant to show). The experimental/control split is the binary one settled in Stage 1 (`conventions/statistics.md`); because pools and references read differently (technical reproducibility/drift vs cross-batch anchoring), a QC figure may further distinguish the control subtypes — e.g. the PCA class coloring gives **each control type its own color**, and the CV plot renders **one figure per control type**.

**Five deliberate exceptions** — the **sample-correlation heatmap** (`lib/figures/sample-correlation`), the **identification-depth bar chart** (`lib/figures/id-depth`), the **missingness completeness curve** (`lib/figures/missingness`), the **dynamic-range per-class overlay** (`lib/figures/dynamic-range` with `class_by`), and the **PCA sample-class coloring** (`lib/figures/pca-plot` colored by sample class — a distinct color per control type): there, controls are shown *with* the experimental samples, distinguished by a **label** (a correlation annotation stripe; a depth-bar color; a curve color; a point color), because the cross-class comparison (do pools sit apart from biology / sit high and tight on depth / run more complete / cover the same dynamic range / project away from the experimental cloud? do references bridge batches?) *is* the deliverable — splitting them apart would destroy it. These are the only QC figures that mix the two classes, and only because they label them; the reviewer treats them as conforming. (The PCA *batch-axis* coloring is **not** among them — there the reference samples are greyed, i.e. rendered separately.)

## QC & exploration figures

The Stage 3 QC report (and the Stage 4 exploration seeds carried from it) is built on seven shipped `lib/figures/` templates, each routing colors through the registry and dual-exporting with a separate legend. **Every QC figure states its processing state** (raw / normalized / batch-corrected) and scale — panel labels on the multi-state plots, title/caption on the single-state ones — so a reader never guesses which stage it depicts. **Batch-corrected panels appear only when batch correction was actually previewed** (a real batch axis with ≥ 2 batches; with one plate they are omitted). That Stage-3 correction is a **preview** — batch-label-only, to display the *effect* of correction for sign-off — distinct from the committed Stage-4 testing decision (`conventions/statistics.md`). To keep the fan-out cheap, the processing-state matrices are **prepared once** (a QC prep-once step materializes them to `results/qc_states/…` via `dataset-io.save_dataset`) and each plot-family **loads** the state it needs rather than re-normalizing / re-running ComBat. **Scale is load-bearing and differs per plot:**
...
- **`cv-plot`** — per-feature CV (std/mean) distributions, overlaid across processing states (raw → normalized → batch-corrected) for the **experimental subset**, and **one figure per control type** (pooled-QC, reference, standard, …) — each control kind is a distinct reading (pool CV = technical reproducibility) and never shares an axis with another class or with the experimental overlay. Computed on the **linear** matrix — the template **hard-refuses a non-linear scale** (`CVScaleError`); the **batch-corrected** state is ComBat output de-logged back to linear via `normalize.to_linear` (a documented approximation — CV read on the linear image of a log-scale correction). The normalized state is CV-able only under a linear normalization (median); MAD/VSN → overlay raw alone. The whole CV family (experimental overlay + per-control-type figures) is **one parameterized script / one `figure-generator` dispatch**.
...
- **`pca-plot`** — PC1/PC2 (+ PC3/PC4) with marginal tests. Produced as a **processing-state series rendered as small multiples** (one panel per state: raw → normalized → batch-corrected — read whether the control samples cluster more tightly after each step, whether a batch-driven split collapses), as **two figures** — one per coloring: by the **batch axis** (reference samples greyable — what the series is read against), and by **sample class with a distinct color per control type** (experimental / pooled-QC / reference / standard / … as *separate* registry categories, the >8 guard applies — **not** a single control-vs-experimental split). The whole PCA family is **one parameterized script / one `figure-generator` dispatch** (two figures, not a six-pack of state×coloring). On the **log** matrix (**warns** on a non-log scale).
...
**A restricted-panel model figure must say so.** Every result figure of the predictive templates (`classification`, `classification-xgboost`, `regression`) automatically appends a **feature-list caveat line** to its title when the model was built on a prior `feature_list=` — `prior feature list · N features`, upgrading to `N of M matched` on a partial match; nothing is added for a whole-proteome run. It is a **mandatory caveat** (applied even to a caller-supplied title, via the shared `_apply_title`) derived from the result's `n_features_requested`/`n_features_matched`, so a panel-restricted result is never mistaken for a whole-proteome one, for free and consistently across every figure. The figure-reviewer checks the note is present whenever the analysis used a feature list.
...
- **`plot_coefficients`** — the **top-N selected features** by |all-data standardized coefficient|, each a diamond at its final coefficient over its **resample IQR**, colored by **selection frequency** (viridis); a line at 0 separates the classes. **Selected-only** — features zeroed in the final model do not appear — so it shares a *skeleton* with the Boruta importance plot (below), not an identical look. The viridis colorbar sits beside the axes (no separate legend).
- **`plot_hyperparameter_heatmap`** — the C × l1_ratio tuning surface (mean inner-CV AUC) with the selected cell boxed; a diagnostic.

The **XGBoost classifier** template (`lib/analysis/classification-xgboost`) ships the **four tree counterparts** of these figures (`lib/figures/classification_xgboost`, also Stage-4). `plot_roc` and `plot_null` are the same as above; the other two diverge because a tree importance is **unsigned**:

- **`plot_importance`** — the **top-N features** by **gain importance**, each a diamond at its all-data gain over its **resample IQR**, colored by **selection frequency** (viridis). **Unsigned** — importances are non-negative, so there is **no zero line** and the axis **starts at 0** (a magnitude view; this is the deliberate divergence from the signed coefficient plot). Selected-only; shares a *skeleton* with the Boruta importance plot. The colorbar sits beside the axes (no separate legend).
- **`plot_hyperparameter_heatmap`** — the **max_depth × learning_rate** tuning surface (mean inner-CV AUC) with the selected cell boxed; the 2-D grid the tree tuner sweeps (the other tree knobs are held constant).

The **regression** template (`lib/analysis/regression`) ships **four** result figures (`lib/figures/regression`), the continuous-outcome counterparts of the classifier figures (also Stage-4, on normalized log data):

- **`plot_predicted_vs_observed`** — the per-sample **true-vs-predicted scatter**, each sample's held-out prediction averaged across the outer nested-CV repeats so it appears once, with a dashed `y = x` reference, a fitted line, and **R² / RMSE / MAE ± SD + N** annotated. Its legend sits **on-axes** (upper-left; the documented exception above) — the ROC's continuous-outcome analogue.
- **`plot_null`** — the **target-shuffle null** R² histogram with the observed R² marked and the empirical p. **Conditional:** it renders only when the null was run (`run_null=True`) and **raises** otherwise (a regression finding without the null is `exploratory`).
- **`plot_coefficients`** — the **top-N selected features** by |all-data standardized coefficient|, diamond = final coef over the **resample IQR**, colored by **selection frequency** (viridis); a line at 0 separates the directions. **Selected-only** — it shares a *skeleton* with the classifier coefficient plot and the Boruta importance plot. The colorbar sits beside the axes (no separate legend).
- **`plot_hyperparameter_heatmap`** — the alpha × l1_ratio tuning surface (mean inner-CV R²) with the selected cell boxed; a diagnostic.

The **Boruta** template (`lib/analysis/boruta`) ships one result figure (`lib/figures/boruta_importance`), also Stage-4 (on normalized log data):

- **`plot_boruta_importance`** — a horizontal **box plot of per-iteration importance** for the top-N ranked features (box + jittered per-iteration scatter). **Accepted** features (Confirmed + Tentative) are colored by their **median importance** on `viridis` (a continuous gradient — no categorical color budget); **Rejected** features are gray; a red dashed vertical line marks the **median shadow threshold** (the noise floor a feature had to beat), and the Confirmed/Tentative/Rejected counts sit in the title. It reads directly as "which features cleared the shadows?" — the all-relevant complement to the classifier's minimal-optimal coefficient plot, with which it shares a *skeleton* (top-N horizontal, colored by a per-feature quantity) but not a look (decision + importance-vs-floor, not a signed coefficient). The importance colorbar sits **beside the axes** and the small shadow-line / rejected key sits **on-axes** (the documented exception), so it passes **no** separate legend image. Domain id prettifying (e.g. UniProt → gene symbol) is not baked in — pass a `label_formatter` in the project copy.

The **enrichment** template (`lib/analysis/enrichment`) ships **three** result figures (`lib/figures/enrichment`, `enrichment_dotplot` / `enrichment_barplot` / `enrichment_manhattan`) — the scientist picks the house style; all three color the ontology **sources** (GO:BP/MF/CC/KEGG) from the registry (an `EnrichmentSource` category, within the >8 guard) and dual-export with a separate legend:

- **`enrichment_dotplot`** — the richest single view: one panel per source; y = term, x = **gene ratio** (query hits in the term / query size), dot **size** = intersection size (# query genes), dot **color** = −log10 corrected p (viridis). The source name is an **in-panel** label (top-left), never a title sitting in the inter-panel gap where it would collide with the tick labels of the panel above, and the **whole key (color scale + the dot-size legend) is the separate legend image** — so nothing overlaps a data point (the layout settled from user feedback on the preview).
- **`enrichment_barplot`** — the top terms across sources as horizontal bars, x = −log10 corrected p, **bar color = source**, gene counts inline, a dashed line at the significance threshold; the source swatches are the separate legend image. Cleanest for a slide or paper.
- **`enrichment_manhattan`** — the g:Profiler-style Manhattan: **every tested term** laid out along x grouped by source, y = −log10 corrected p, the significance line, and the most-significant terms labelled **collision-free** (via `textalloc`). Needs the full tested-term set, so it **raises** unless the result was produced with `enrich(all_results=True)` (the default).

An empty significant set renders an **empty-state figure** (a "no significant terms" placeholder) rather than crashing a batch.
...
| Each QC figure labels its processing state (raw / normalized / batch-corrected) + scale; a batch-corrected panel appears only when batch correction was previewed (≥2-batch axis) | **Figure-reviewer** |
| CV / PCA / abundance-boxplot show the full processing-state series; PCA + CV distinguish each control type (per-type PCA color / per-type CV figure); batch-corrected CV de-logged via `normalize.to_linear` | **Figure-reviewer** (+ `cv-plot` refuses log, so the batch-corrected state must be de-logged) |
...
| A model figure built on a prior `feature_list=` states it in the title (`prior feature list · N features` / `N of M matched`) | **Figure-reviewer** (+ the classification/xgboost/regression figure templates append it automatically in `_apply_title` from the result's `n_features_requested`) |
...
| XGBoost classifier figures consistent with the result (ROC ±SD + null as above; importance plot **unsigned** — no zero line, axis from 0 — colored by selection frequency; max_depth × learning_rate heatmap selected cell boxed) | **Figure-reviewer** (+ `lib/figures/classification_xgboost`; `plot_null` raises without a null) |
| Regression figures consistent with the result (predicted-vs-observed scatter with y=x + fit + R²/RMSE/MAE, legend on-axes; null figure present only when the null ran; coefficient plot selected-only + colored by selection frequency; heatmap selected cell boxed) | **Figure-reviewer** (+ `lib/figures/regression`; `plot_null` raises without a null) |
| Boruta importance figure consistent with the result (accepted colored by median importance on viridis, rejected gray, shadow-threshold line drawn, C/T/R counts shown; colorbar + key on-axes, no separate legend image) | **Figure-reviewer** (+ `lib/figures/boruta_importance`) |
