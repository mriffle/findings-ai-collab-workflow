...
The Stage 3 QC report (and the Stage 4 exploration seeds carried from it) is built on four shipped `lib/figures/` templates, each routing colors through the registry and dual-exporting with a separate legend. **Scale is load-bearing and differs per plot:**
