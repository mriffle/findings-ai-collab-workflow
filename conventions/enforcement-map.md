...
| QC/descriptive figures render control samples separately from experimental | 06 | **Figure-reviewer** | reviewer confirms controls are shown in their own panels or visibly distinct, not silently pooled into the experimental distributions. Four documented exceptions show them together *because labeled*: the `sample-correlation` heatmap (Sample Type stripe), the `id-depth` bar chart (bar color), the `missingness` completeness curve (curve color), and the `dynamic-range` per-class overlay (curve color) — the cross-class comparison is the deliverable |
