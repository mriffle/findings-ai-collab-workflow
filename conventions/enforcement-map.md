...
| QC/descriptive figures render control samples separately from experimental | 06 | **Figure-reviewer** | reviewer confirms controls are shown in their own panels or visibly distinct, not silently pooled into the experimental distributions. Three documented exceptions show them together *because labeled*: the `sample-correlation` heatmap (Sample Type stripe), the `id-depth` bar chart (bar color), and the `missingness` completeness curve (curve color) — the cross-class comparison is the deliverable |
