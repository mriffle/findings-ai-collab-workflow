...
| Script not promoted until **strict lint + types** pass | 05.4 | **Hook** | `guard_promotion.sh` — `ruff check` (strict rule set) + `mypy` strict on `scripts/promoted/*.py`; strictness from the project config (PreToolUse blocks; PostToolUse warns) |
...
- Several rows credit a **`lib/`-seeded guard** (the >8-category raising guard; the leakage-safe / moderated-model template defaults) as a deterministic co-enforcer. **Those templates are not built yet — phase-E work.** Until they ship, the **reviewer agent named in the row is the sole active enforcer**; the rows carry a *pending phase E* marker so the map never overstates current coverage.
- The shipped templates' **runtime guards** (the `normalize` scale-tag check; the `batch-correct-combat` log-scale / batch-label-only / confounding checks) are real enforcers, but of a fourth kind: they live *inside the template code*, so they only protect a project that **keeps the guard when adapting the copy**. That is why the reviewer agent is still named as co-enforcer on those rows — the reviewer is what catches a project that strips the guard. A runtime guard is not a substitute for the review.
