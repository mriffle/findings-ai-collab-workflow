...
**Classification / regression / selection evidence (a distinct shape).** A finding from a predictive-model or selection template — `lib/analysis/classification`, `lib/analysis/classification-svm`, `lib/analysis/classification-lda`, `lib/analysis/classification-xgboost`, `lib/analysis/regression`, and Boruta — does **not** fit the effect + CI + corrected-p mold, and forcing it to would be dishonest. Its evidence is two coupled parts, both recorded:

- **Run-level performance vs a null.** The cross-validated metric with its fold spread — a **classifier's** balanced accuracy / ROC-AUC ± SD (plus average precision under imbalance), a **regressor's** held-out R² / RMSE / MAE ± SD — *and* the **shuffle null** it is tested against (label-shuffle for a classifier, target-shuffle for a regressor; the null distribution's summary + the **empirical p**). Record the generalization target (`"on unseen samples/individuals/batches"`) and the CV design. **Without the null the finding is `exploratory`** — the coefficients/importances are not licensed; running it is what makes the finding eligible for `validated` (`conventions/statistics.md`). This is the **expected state of a first-pass model finding**: the null is a deliberate second pass (≈15,000 extra fits — minutes for the shrinkage LDA, which follows in the same sitting; hours for the tree model, whose first null pass may use fewer permutations; `conventions/statistics.md`), so such a finding is recorded now, with the metric and the stability-read coefficients, carrying the caveat *"not tested against a null"* and the null named as its outstanding **follow-up**. It is a legitimate finding, not a half-written one — but the caveat and the follow-up are **required**, since the null is also the leakage check.
- **Per-feature selection evidence.** For each selected feature, the **all-data point estimate** with its cross-resample **stability read** — its **selection frequency** and the estimate's resample spread. The estimate's form follows the estimator: a **signed standardized coefficient** (with **sign consistency**) for a linear model (elastic-net classifier/regressor, linear-SVM classifier, shrinkage-LDA classifier), or an **unsigned importance** (`total_gain` by default; no sign, so no sign consistency) for a **gradient-boosted-tree** classifier (`classification-xgboost`) — a magnitude that says a feature was *useful for splitting*, not a direction. For a **dense** model (the linear SVM and the shrinkage LDA — every weight is non-zero) the stability read is the **top-k membership frequency** (fraction of stability resamples in which the feature ranks in the top *k* by |weight|) in place of a selection frequency, with the same sign consistency + resample spread. Either way this is a *selection*, not a per-feature significance test — there is **no per-feature q**, and the *minimal-optimal* caveat (a low selection frequency ≠ unimportant; correlated features) is carried in the finding's `caveats`.
...
figures:
  ...
  - png:        "figures/analysis/differential-abundance/drug-a-vs-control/0042-volcano.png"        # 300 DPI raster; review + embed target
    ...
    result_id:    "fp-a1b2c3…"                     # for a figure rendered from a cached CPU-heavy result (classification/svm/xgboost/regression/boruta); null otherwise
