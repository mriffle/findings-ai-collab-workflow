...
- **Show, don't tell.** A Results claim whose source finding illustrated it must stay **illustrated**: the figure is embedded inline, not merely referenced, **with its legend image beneath it** whenever the finding's entry carries one, and the prose **explains** it (what is plotted, where to look, what it establishes). Flag a claim written as bare assertion when its finding carries a figure for it, flag an embedded figure shown without its legend, and flag an embedded figure the prose never reads (`conventions/reporting.md`; `conventions/findings.md` §9).
