...
- **Two linear classifiers — the claim rests on the paired comparison.** When both the elastic net and the linear SVM were run, a preference for one must rest on the **paired per-fold comparison** at one seed (per-fold differences, win/loss/tie counts), never on the two mean AUCs; a difference within seed noise (~±0.03 at n ≈ 40) must be reported as a **tie** settled by the reading the scientist needs, and both results' fingerprints must be recorded in the finding (`conventions/statistics.md`, *Choosing between the two linear classifiers*).
