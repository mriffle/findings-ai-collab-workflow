...
- **Two linear classifiers — the claim rests on the paired comparison.** When both the elastic net and the linear SVM were run, a preference for one must rest on the **paired per-fold comparison** on shared splits (per-fold differences, win/loss/tie counts), never on the two mean AUCs. Check four things: (1) the **reading was stated before the second model ran** (sparse minimal set vs dense module weights) — a tie settled by a reading chosen after seeing both feature lists is motivated reasoning, FAIL; (2) the SVM run carried **no unresolved `CGridEdgeWarning` / `TuningNoiseWarning`** — a comparison against a mis-tuned SVM is an artifact of the grid; (3) a preference is claimed only if its **sign holds across a few seeds** — otherwise it is reported as a **tie**; and any p respects that CV folds are **dependent** — a plain Wilcoxon over the fold pairs is anti-conservative, so a decisive p uses the corrected resampled t-test (Nadeau & Bengio 2003) or the Wilcoxon is labelled *indicative*; (4) **both AUCs and both fingerprints are recorded** in the finding, not only the winner's (`conventions/statistics.md`, *Choosing between the two linear classifiers*).
