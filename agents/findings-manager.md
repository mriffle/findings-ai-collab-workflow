...
3. **Show, don't tell — run the claim→figure coverage pass.** Figures are **first-class evidence** in a finding, not decoration (`conventions/findings.md` §2.4). Work **claim by claim**, not figure by figure:
   - **a. Coverage.** Enumerate the claims the finding makes about *this dataset* (in `summary`, `verdict`, `evidence`, and any caveat). For each, ask **"what figure shows this?"** and match it to a figure — one the orchestrator handed you, or an existing artifact under `figures/` (in its phase/family directory — `conventions/visualization.md`, *Where figures live*) produced by this finding's analysis. A figure of something *adjacent* does not cover a claim. Any claim left **uncovered that could be shown is reported back to the orchestrator** so a `figure-generator` can be commissioned (§ Output contract) — never quietly record a showable claim as bare prose. (A claim that genuinely cannot be shown — a bare count, an identifier, a design fact — simply carries no figure, and that is a legitimate outcome; interpretive claims in `## Discussion` rest on `references`, not on figures.)
   - **b. List** each covered figure in the `figures` frontmatter with its `png`/`svg`, its `legend_png`/`legend_svg` (whenever the figure has a legend image — omit them when the generator reports the key is on-axes), `caption`, and **its own producing script + input** (`script: { path, commit }`, `data_version`, `result_id` when rendered from a cached result, optional `params`) — this per-figure provenance may differ from the finding-level `provenance.script`, and is what makes *that* figure regenerable on its own.
   - **c. Embed *and explain*** it inline in the body (normally in `## Evidence`, next to the numbers it illustrates; a QC/design-caveat figure may sit in `## Caveats`), in the **five-part pattern** of `conventions/findings.md` §9: **the claim** in prose → the markdown image `![<caption>](../figures/analysis/<family>/<label>/<NNNN>-<name>.png)` — **body links are relative to `findings/`**, so a figure needs the `../`, and the path carries the figure's phase/family directory (frontmatter paths stay project-root-relative; `conventions/findings.md` §2.7) → **the legend image directly beneath it** (`![Legend for Figure N — …](../figures/analysis/<family>/<label>/<NNNN>-<name>.legend.png)`) whenever the figure has one — a legend is essential to reading the figure, so it is **shown, never cited as a path** → a **one-line provenance pointer** (producing script + commit + data version / result id) → **the reading**: one or two sentences saying what is plotted, where to look, and what that establishes for the claim. **The reading is required** — an embedded figure the text never explains leaves the finding incomplete, and a caption does not count as an explanation.

   The frontmatter `figures` list and the body's inline images **must correspond** — every listed figure is embedded, every embedded figure is listed, and the same holds for legends: every listed `legend_png` is embedded beneath its figure, every embedded legend image is some entry's `legend_png` (a hook backstops **both** directions for both). If the orchestrator names or hands you a relevant figure but you were not given its producing script + input, do **not** silently drop it: embed it and **flag the missing per-figure provenance** back to the orchestrator. An early `candidate` with nothing rendered yet simply carries `figures: []` — add them (listed, embedded, explained) as soon as they exist, and report the coverage gap in the meantime.
4. **Link every cross-reference.** Wherever the body names another finding, make it a **link**: `[finding 0031](0031-sex-confounded-with-group.md)` — target is the sibling filename `<id>-<slug>.md`, resolved from the manifest's `ID` + `Slug` columns (never a directory scan); link text is free prose that must contain the zero-padded id (`conventions/findings.md` §2.7). Every `relationships` edge target is mentioned and linked in `## Related findings` — an edge with no narrative is an unexplained edge. Conversely, a body mention that is **not** in `relationships` is a **flag, not an error**: report it as a possible missing edge (§ Output contract) and let the scientist decide the type; never invent the edge yourself. Refuse to write a link to an id that has no finding file.
5. **Novelty + relationship pass** (do this for every new finding): scan the manifest for findings with overlapping `entities` and similar claims. Decide whether the new finding is genuinely novel, a near-duplicate (recommend merge/close), or stands in a typed relationship to existing findings (`supports`/`refines`/`contradicts`/`supersedes`/`closes`/`relates_to`). **Propose** edges; assert them per §C.
6. Update `findings/manifest.md`: append the projected table row, set `next_id += 1` (frontmatter), refresh `generated`.
7. Return the assigned id and a one-line summary so the orchestrator can give the scientist the non-disruptive notice ("recorded as finding 00NN"). Recording is low-bar and biased toward capturing too much (doc 03.9) — clutter is cheaper than lost insight.

### B. Update a finding

Edit the file's frontmatter/body, bump `updated`, and re-project the changed fields into the manifest. Never let the manifest drift from the file. When a figure is added to (or regenerated for) a finding, keep the `figures` frontmatter and the body's inline images **in lockstep** — a newly available figure is listed (with its per-figure provenance), embedded inline **with its legend image beneath it**, *and* given its reading in the prose (§A.3); a removed/superseded one is dropped from both, legend included. **Re-run the coverage pass (§A.3a) whenever a claim is added or changed** — a new claim that can be shown needs a figure just as much as the original ones.
...
- **Link integrity:** every cross-reference link in every body resolves to an existing finding file. Because a link embeds the **slug**, a renamed file breaks inbound links — a rename is not done until they are repaired (resolve from the manifest's `ID` + `Slug`). Keep, but **flag**, a link whose target has since been `invalidated`/`superseded`: the citation is part of the record, but the citing text may now be wrong.
...
- **No dangling edges — and no dangling links.** Refuse an edge to a nonexistent target, and never write a cross-reference link to an id with no finding file. Every mention of another finding in a body is a **link** (§A.4); body links are relative to `findings/`.
