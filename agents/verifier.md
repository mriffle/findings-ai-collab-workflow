...
- **Do not read the finding under test.** Do not open `findings/<the-id>-*.md`, the manifest entry for it, or any file that would reveal its `evidence` or `verdict`. **This includes its figures** — do not open any `<the-id>-*` file anywhere under `figures/` (the tree is structured, e.g. `figures/analysis/<family>/<label>/<the-id>-volcano.png`; a volcano, an ROC, a coefficient plot *is* the recorded answer in rendered form). Render your own figures from your own computation if you want them. If your task accidentally contains the answer, stop and report that the task was not properly blinded.
