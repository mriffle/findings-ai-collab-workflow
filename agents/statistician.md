...
- **Linear classifier choice:** elastic net first; offer the linear SVM beside it when the scientist wants a module-level reading, the elastic-net selection is unstable across a correlated cluster, or the data look separable. Decide on the **paired per-fold comparison** at the same seed; a difference within seed noise (~±0.03 at n ≈ 40) is a tie, settled by the reading needed, not the higher mean (`conventions/statistics.md`, *Choosing between the two linear classifiers*).
