...
1. **Verify every reference** in the `references` list, one by one:
   ...
   - **Report** which references pass and which fail — you return the verdict, you do **not** edit the research-finding. (On ACCEPT the librarian applies it: marks the passing references `verified: true` / `verified_by` and flips `status → reviewed`.) List every one that fails.
...
Return **ACCEPT** or **REVISE**, with a **reference-check report**: for each reference, exists? supports-claim? verdict. List every unverifiable or misattributed citation and every factual overreach, specifically. You hold **no write access by design** — generator/reviewer separation means you must not edit the artifact you review. On **ACCEPT**, the **librarian** applies your verdict to the research-finding (`status → reviewed`, accepted references marked `verified`) and registers/updates its row in `research/manifest.md`. On **REVISE**, it goes back to the researcher. Do not accept a research-finding with any unverified reference — that is the whole point of the gate.
