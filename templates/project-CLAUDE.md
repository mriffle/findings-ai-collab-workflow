...
| `state/METADATA.md` | Verified column meanings, design, confounds/imbalances, the experimental/control sample classification, the data-checkable annotations (which columns a near-binary marker can check, and the marker), join key; data-version stamped (Stage 1). |
...
No bare p-values (always effect size + CI + a **named** multiple-testing correction); report all tests run; **biological analysis runs on the experimental samples only** (control pools/references are identified in Stage 1, excluded from every contrast, and the exclusion is recorded in provenance); **annotations are checked against the data at the integrity gate where a near-binary marker exists** (sex ↔ Y-linked proteins, genotype ↔ a transgene) — **conservatively**: the marker must prove itself before it may question a label, only an unambiguous discordance is flagged, a flag is a question and never "your metadata is wrong", and the annotation stands unless the scientist decides otherwise (a correction is a documented loader override, never an edit to the read-only metadata file); prefer canonical/moderated models for differential abundance; normalization is a recorded choice (median by default; respect the `scale` tag — never double-log); **batch correction is batch-label-only** (never hand ComBat the covariate of interest) and the key analysis is reported **corrected and uncorrected** (survival-of-correction is the test under confounding); **no data leakage** (learn preprocessing inside CV folds; the inner tuning folds are grouped whenever the outer folds are); match CV to the generalization target; for a classifier/regressor, run the **first pass without the shuffle null** (results fast) and **propose the null as the immediate follow-up** — until it runs the finding is `exploratory` and the coefficients are flagged "not tested against a null" (the shrinkage LDA's null is minutes: run it in the same sitting; the tree model's is hours); **the elastic net is the default classifier** and an alternative (the SVM, the tuning-free shrinkage LDA, or the tree) runs only on a named trigger and is compared **paired per fold against the elastic net** with the `classifier-comparison` template, never on the two means; treat small-n as exploratory. Assumptions are hypotheses — test them in code and record the result. Full text in the plugin's `conventions/`.
