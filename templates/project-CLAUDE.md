...
- **A finding is a comprehensive, standalone artifact — figures go *in* it.** Every figure relevant to a finding is **embedded inline** in the finding (not merely left sitting in `figures/`), and each carries its own **producing script + input** (path + commit, data version, and the cached-result id when applicable). Hand the findings-manager the figures a discovery is about along with their provenance; the reader must never have to go track down a figure that exists. (Schema: the plugin's `conventions/findings.md` §2.4; a hook backstops that a listed figure is actually embedded.)
