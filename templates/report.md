...
<!--
  ...
  - caveat findings (kind: caveat) render as Limitations and attach to the results they qualify (relates_to).
...
findings are preserved; exploratory results are framed as hypothesis-generating. Include a
**Limitations** paragraph that surfaces the cohort's caveat findings (kind: caveat — class
imbalance, skew, confounding), attaching each to the result it qualifies (its relates_to edge).>
