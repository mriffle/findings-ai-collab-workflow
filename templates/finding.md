...
provenance:
  ...
  result_id: null                       # for a finding built on a cached CPU-heavy result (classification/svm/lda/xgboost/regression/boruta): the result-io fingerprint it was rendered from (conventions/results-cache.md). Pins the exact cached result; null otherwise.
...
figures: []
  ...
  #     data_version: "<sha256:... or version stamp>",
  #     result_id: null,          # the result-io fingerprint if rendered from a cached result (classification/svm/lda/xgboost/regression/boruta); null otherwise
